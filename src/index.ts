	...
	class IccApiPouchedImpl implements IccApiPouched {
		...
		constructor(host: string, username: string, password: string, headers?: { [key: string]: string }, lastSync?: number, localDatabaseName?: string) {
			...
			const ddoc = {
				...
				views: {
					by_searchString: {
						map: function(doc: PatientStubCore & PouchDB.Core.ExistingDocument<PatientStubCore>) {
							const emitNormalizedSubstrings = (txt: string, doc: PatientStubCore & PouchDB.Core.ExistingDocument<PatientStubCore>, latinMap: { [key: string]: string }) => {
			...
			if (currentUser) {
				...
				_.sortBy(patList, pat => -(pat.modified || 0)).reduce(async (prev: Promise<void>, remotePat) => {
					...
					const filtered = Object.assign(keysToKeep
						...
						}, {}), { _id: remotePat.id, _upstreamRev: remotePat.rev })

					if (filtered._id) {
						const localPat: any = await this._database.get(filtered._id)
						if (localPat) {
							const localRev = localPat._upstreamRev ? +(localPat._upstreamRev.split('-')[0]) : 0
							const remoteRev = remotePat.rev ? +(remotePat.rev.split('-')[0]) : 0
							if (localRev < remoteRev) {
								await this._database.put(Object.assign(filtered, { _rev: localPat._rev }))
							}
						} else {
